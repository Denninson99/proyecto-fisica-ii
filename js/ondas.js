...
function setup() {
  const canvas = createCanvas(760, 800);
...
function dibujarEncabezadoCanvas(e) {
  noStroke();
  fill(255);
  rect(38, 20, width - 76, 200, 18);

  fill(15, 23, 42);
  textAlign(CENTER);
  textStyle(BOLD);
  textSize(22);
  text('Ondas electromagnéticas en vacío y materiales', width / 2, 56);

  textStyle(NORMAL);
  textSize(12);
  fill(76, 91, 112);
  text('Se compara la onda en vacío con la onda dentro del material.', width / 2, 86);
  text('La frecuencia permanece constante; cambian la rapidez y la longitud de onda.', width / 2, 106);
  ...
  dibujarMiniDato(83, 155, 145, 'Material', material);
  dibujarMiniDato(244, 155, 170, 'Frecuencia', formatearFrecuencia(e.frecuencia));
  dibujarMiniDato(430, 155, 108, 'v/c', e.relacion.toFixed(4));
  dibujarMiniDato(554, 155, 125, 'n', e.indice.toFixed(3));

  fill(100, 116, 139);
  textSize(10.5);
  textAlign(CENTER);
  text(detalle, width / 2, 202);
}
...
function dibujarComparacionOndas(e) {
  const inicioX = 125;
  const finX = width - 92;
  const vacioBloqueY = 245;
  const materialBloqueY = 455;
  const altoBloque = 190;
  const vacioY = vacioBloqueY + 88;
  const materialY = materialBloqueY + 88;
  const separacionCampos = 68;
  const maxIndice = Math.sqrt(100 * 2000);
  const lambdaVacioPx = 250;
  const lambdaMaterialPx = constrain(lambdaVacioPx / e.indice, 70, lambdaVacioPx);
  const avance = frameCount * map(e.relacion, 1 / maxIndice, 1, 0.012, 0.070);

  dibujarBloqueMedio(58, vacioBloqueY, width - 116, altoBloque, 'Vacío', 'λ₀ = c / f', formatearLongitud(e.lambdaVacio));
  dibujarBloqueMedio(58, materialBloqueY, width - 116, altoBloque, 'Material', 'λ = v / f', formatearLongitud(e.lambdaMaterial));

  dibujarEje(inicioX, finX, vacioY, 'Dirección de propagación →');
  dibujarEje(inicioX, finX, materialY, 'Dirección de propagación →');

  dibujarOnda(inicioX, finX, vacioY, lambdaVacioPx, 24, avance, color(11, 117, 255), 4);
  dibujarOnda(inicioX, finX, vacioY + separacionCampos, lambdaVacioPx, 17, avance, color(20, 140, 80), 3);
  dibujarOnda(inicioX, finX, materialY, lambdaMaterialPx, 24, avance, color(11, 117, 255), 4);
  dibujarOnda(inicioX, finX, materialY + separacionCampos, lambdaMaterialPx, 17, avance, color(20, 140, 80), 3);

  dibujarEtiquetaCampo(78, vacioY, 'E', color(11, 117, 255));
  dibujarEtiquetaCampo(78, vacioY + separacionCampos, 'B', color(20, 140, 80));
  dibujarEtiquetaCampo(78, materialY, 'E', color(11, 117, 255));
  dibujarEtiquetaCampo(78, materialY + separacionCampos, 'B', color(20, 140, 80));
  ...
  textSize(12);
  fill(51, 65, 85);
  text('E y B están en fase. En el material la rapidez baja y la longitud de onda se reduce.', width / 2, 675);
}
...
function dibujarBloqueMedio(x, y, w, h, titulo, formula, lambdaTexto) {
  ...
  textStyle(BOLD);
  textSize(15);
  text(titulo, x + 20, y + 30);

  fill(71, 85, 105);
  textStyle(NORMAL);
  textSize(11.5);
  text(`${formula}  →  ${lambdaTexto}`, x + 20, y + 52);
}
...
function dibujarPanelCanvasOndas(e) {
  ...
  textStyle(BOLD);
  textSize(15);
  text('Cálculo actual', x + 22, y + 27);

  textStyle(NORMAL);
  textSize(12);
  fill(71, 85, 105);
  text(`f = ${formatearFrecuencia(e.frecuencia)}   |   K = ${e.K.toFixed(2)}   |   Km = ${formatearNumero(e.Km)}   |   v = ${e.v.toExponential(3)} m/s`, x + 22, y + 52);
  text(`λ₀ = ${formatearLongitud(e.lambdaVacio)}   |   λ material = ${formatearLongitud(e.lambdaMaterial)}   |   E = vB`, x + 22, y + 72);
}
