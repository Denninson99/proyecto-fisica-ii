...
function setup() {
  const canvas = createCanvas(760, 735);
...
function dibujarEncabezadoCanvas(e) {
  noStroke();
  fill(255);
  rect(38, 20, width - 76, 145, 18);

  fill(15, 23, 42);
  textAlign(CENTER);
  textStyle(BOLD);
  textSize(22);
  text('Ondas electromagnéticas en vacío y materiales', width / 2, 53);

  textStyle(NORMAL);
  textSize(12);
  fill(76, 91, 112);
  text('Se compara la onda en vacío con la onda dentro del material.', width / 2, 78);
  text('La frecuencia permanece constante; cambian la rapidez y la longitud de onda.', width / 2, 96);
  ...
  dibujarMiniDato(83, 124, 145, 'Material', material);
  dibujarMiniDato(244, 124, 170, 'Frecuencia', formatearFrecuencia(e.frecuencia));
  dibujarMiniDato(430, 124, 108, 'v/c', e.relacion.toFixed(4));
  dibujarMiniDato(554, 124, 125, 'n', e.indice.toFixed(3));

  fill(100, 116, 139);
  textSize(10.5);
  textAlign(CENTER);
  text(detalle, width / 2, 155);
}
...
function dibujarComparacionOndas(e) {
  const inicioX = 125;
  const finX = width - 92;
  const vacioBloqueY = 185;
  const materialBloqueY = 405;
  const vacioY = vacioBloqueY + 98;
  const materialY = materialBloqueY + 98;
  ...
  dibujarBloqueMedio(58, vacioBloqueY, width - 116, 175, 'Vacío', 'λ₀ = c / f', formatearLongitud(e.lambdaVacio));
  dibujarBloqueMedio(58, materialBloqueY, width - 116, 175, 'Material', 'λ = v / f', formatearLongitud(e.lambdaMaterial));

  dibujarEje(inicioX, finX, vacioY, 'Dirección de propagación →');
  dibujarEje(inicioX, finX, materialY, 'Dirección de propagación →');

  dibujarOnda(inicioX, finX, vacioY, lambdaVacioPx, 28, avance, color(11, 117, 255), 4);
  dibujarOnda(inicioX, finX, vacioY + 68, lambdaVacioPx, 20, avance, color(20, 140, 80), 3);
  dibujarOnda(inicioX, finX, materialY, lambdaMaterialPx, 28, avance, color(11, 117, 255), 4);
  dibujarOnda(inicioX, finX, materialY + 68, lambdaMaterialPx, 20, avance, color(20, 140, 80), 3);

  dibujarEtiquetaCampo(78, vacioY, 'E', color(11, 117, 255));
  dibujarEtiquetaCampo(78, vacioY + 68, 'B', color(20, 140, 80));
  dibujarEtiquetaCampo(78, materialY, 'E', color(11, 117, 255));
  dibujarEtiquetaCampo(78, materialY + 68, 'B', color(20, 140, 80));
  ...
  textSize(12);
  fill(51, 65, 85);
  text('E y B están en fase. En el material la rapidez baja y la longitud de onda se reduce.', width / 2, 600);
}
